...
class DGS
{
  ...
  public:
    ...
    int setBC(char *BC);  //Initializes DGS with all information from barcode, only way to set Span
    ...
    long getConc(char c = 'p');  //Reads concentration from current data array 'p' ppb or 'c' counts
    long getTemp(char u = 'C');  //Reads temperature from current data array 'C' for Celsius or 'F' for Fahrenheit
    long getRh(void); //Reads Rh from current data array
